...
//#include <machdep/ppc/fpu_protos.h>
...
#define	__inline__	/* lion@apple.com 2/12/97 .. */
...
#if	DEBUG
		kprintf("EXCEPTION NOT HANDLED: Out of range.\n");
		kprintf("dsisr=%X, dar=%X\n",dsisr, dar);
		kprintf("table=%X\n",DSISR_BITS_15_16(dsisr));
		kprintf("table->size=%X\n", table->size);
...
#if	DEBUG
		kprintf("EXCEPTION NOT HANDLED: Inst out of table range.\n");
		kprintf("table=%X\n",DSISR_BITS_15_16(dsisr));
		kprintf("entry=%X\n",DSISR_BITS_17_21(dsisr));
#endif
		goto out;
...
#if	0 && DEBUG
	kprintf("Alignment exception: %s %d,0x%x (r%d/w%d) (tmp %x/%x)\n",
	       entry->name, DSISR_BITS_REG(dsisr),
	       dar, entry->a_readbytes, entry->a_writebytes, 
	       align_buffer[0], align_buffer[1]);
	kprintf("    pc=(0x%08X), msr=(0x%X)",ssp->srr0, ssp->srr1);
#endif

#ifdef MACH_OSF30_VERSION
...
out:
#if	0 && DEBUG
	kprintf("ALIGNMENT EXCEPTION: (dsisr 0x%x) table %d 0x%x\n",
