...
/*	Determines whether it is OK to use floating point support in
	kernel code.
*/
#define	USE_FLOATING_POINT_IN_KERNEL	1
...
#define KERNEL_SEG_REG1_VALUE 0x20000001 /* T=0,Ks=0,Ku=1 PPC_SID_KERNEL=0*/
...
/* Don't do it this way.... so pex can be happy */
//#define SR_COPYIN	12
 ...
 */
        ...
        /*	Use eieio as a memmory barrier to order stores.
...
extern __inline__ void tlbsync()
{
...
#define PROCESSOR_VERSION_602	5
#define PROCESSOR_VERSION_603e	6
#define PROCESSOR_VERSION_603ev	7
#define PROCESSOR_VERSION_750   8
#define PROCESSOR_VERSION_604e	9
#define PROCESSOR_VERSION_604ev	10	/* MachV */

extern void mtmsr(unsigned int val);

extern __inline__ void mtmsr(unsigned int val)
{
...
#define mtsr(SR, REG)												\
	__asm__ volatile("mtsr sr%0, %1" : : "i" (SR), "r" (REG));
...
extern unsigned int mfdec(void);
...
{
        __asm__ volatile("mtspr 1017,%0" : : "r" (val));
        return;
}
...
{
        unsigned int result;
        __asm__ volatile("mfspr %0,1017" : "=r" (result));
        return result;
}
...
extern void dcbf(unsigned int addr);

extern __inline__ void dcbf(unsigned int addr)
{
