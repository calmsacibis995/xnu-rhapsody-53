 ...
 *
 * vgapriv.h - Private definitions used by vga ps driver implementation.
 *
 *
 * HISTORY
 * 28 Sep 92	Gary Crum
...
// TO DO: Color palette setting will be moved into kernel driver
// and change along with Mike Paquette's work.
...
#define zvga_reg_out(reg,indx,data)				\
	{							\
	OUTB(WRIT_##reg##_ADDR, indx)				\
	OUTB(WRIT_##reg##_DATA, data)				\
	}

#define zvga_reg__in(reg,indx,data)				\
	{							\
	OUTB(WRIT_##reg##_ADDR, indx)				\
...
extern void vga_read_bpp4planar_to_bpp2packed32(unsigned short *fb, unsigned int *dst);
extern void vga_write_bpp2packed32_to_bpp4planar(unsigned int *dst, unsigned short *fb);

// Global variables
// TODO: Eliminate the need for these.
extern unsigned int vgapriv_bpl;
extern unsigned int vgapriv_vgamode;
extern unsigned int *vgapriv_vgaAddr;
extern unsigned int *vgapriv_vgaVirtAddr;
extern int colr_mode;


// Alternate VGA routines

// Modes that we are interested in...
// WG_foo are Wingine modes
// VGA_foo are standard VGA modes.

#define	WG_MODE_40	0	// 640x480, 555	Non-interlaced, 50MHz dot clock
#define	WG_MODE_42	1	// 800x600, 555	Non-interlaced, 75MHz dot clock
#define	WG_MODE_50	2	// 640x480, 888 Non-interlaced, 75MHz dot clock
#define	WG_MODE_51	3	// 1Kx768, 555 Non-interlaced, 75MHz dot clock
#define	VGA_MODE_3	4	// 80x25 Alphanumeric mode
#define VGA_MODE_12	5	// 640x480x4 VGA mode
#define	N_MODES		6	// Total number of modes

/*
 * VGASetStdRegs() sets the standard VGA registers based on the supplied mode.
 * The modes are chosen from the above list. New modes can be added by adding
 * to the tables in vga_modes.c.  This routine is usually only a subset of what
 * needs to be done for enhanced modes.
 */
extern int VGASetStdRegs(int mode);

extern void fill_64K_plane(short colr);
extern void save_colr_data();
extern void save_font_data();
extern void set_linr_mono(char max);
extern void get_addr_range(short **segment_st, short **segment_nd);
extern void select_write_segment(char num);
extern void select_read_segment(char num);
