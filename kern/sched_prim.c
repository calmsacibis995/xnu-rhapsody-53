...
#define CHECKRQ(rq, s)		\
    if (runq_debug) checkrq((rq), (s))

#define THREAD_CHECK(th, rq)	\
    if (runq_debug) thread_check((th), (rq))
	...
	boolean_t	interruptible)
{
	...
	thread_block_with_continuation((void (*)()) 0);	/* block ourselves */
}
	...
	register processor_t myprocessor)
{
	...
	if (myprocessor->runq.count > 0) {
		...
		if (pset->runq.count == 0 ||
		    pset->runq.high < thread->sched_pri) {
			/*
			 ...
			 */
			q = pset->runq.runq + pset->runq.high;
	...
	register thread_t new_thread)
{
	    ...
	    (continuation != (void (*)()) 0))
	{
		...
		case TH_SWAPPED:
			...
			default:
			    printf("illegal handoff thread state 0x%x\n",
				   		old_thread->state);
			    panic("thread_invoke");
		    }
		    thread_unlock(old_thread);
...
void thread_block_with_continuation(
	continuation_t	continuation)
{
	register thread_t thread = current_thread();
...
{
	static int recompute_priority_ticks;
#if	SIMPLE_CLOCK
	int	new_usec;
#endif	/* SIMPLE_CLOCK */

	if (recompute_priority_ticks-- > 0)
		return;
...
retry:
		...
		if (state == PROCESSOR_DISPATCHING) {
			...
			*threadp = (volatile thread_t) THREAD_NULL;
			...
			thread_block_with_continuation(idle_thread_continue);
		}
...
void sched_thread_continue(void)
{
    ...
    thread_block_with_continuation(sched_thread_continue);
    /*NOTREACHED*/
}
	...
	char		*msg)
{
	...
	if (j != rq->count)
	    panic("checkrq: count %d wrong at %s (should be %d)",
		  				j, msg, rq->count);
	if (rq->count != 0 && high > rq->high)
	    panic("checkrq: high %d wrong at %s (should be %d)",
		  				high, msg, rq->high);
}
	...
	register run_queue_t	rq)
{
	...
	whichq = th->sched_pri;
	if (whichq >= NRQS) {
		panic("thread_check: priority %d too high\n", whichq);
		whichq = NRQS-1;
	}
...
{
    thread_block_with_continuation((continuation_t) 0);
}
