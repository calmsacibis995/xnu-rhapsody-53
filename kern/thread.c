...
extern zone_t	u_thread_zone;	/* UNIX */
int nthreads;
...
#import <kernel_stack.h>
#if	KERNEL_STACK
#import <kern/kernel_stack.h>
#ifndef	MACHINE_STACK
#define	MACHINE_STACK	1
#else	/* MACHINE_STACK */
#error	MACHINE_STACK and KERNEL_STACK are mutually exclusive
#endif	/* MACHINE_STACK */
#endif	/* KERNEL_STACK */
	...
	thread_t	*child_thread)		/* OUT */
{
	...
	*new_thread = thread_template;
	 ...
	 */
	new_thread->_uthread = (struct uthread *) zalloc(u_thread_zone);

//	uarea_zero(new_thread);		/* XXX */
...
#if	KERNEL_STACK
	if (thread->stack_privilege != 0)
		freeStack(thread->stack_privilege);	/* XXX */
#endif	/* KERNEL_STACK */
