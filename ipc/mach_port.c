	...
	mach_msg_type_number_t	*typesCnt)
{
...
#if	MACH_OLD_VM_COPY
	vm_offset_t memory1;	/* copied-in memory, for names */
	vm_offset_t memory2;	/* copied-in memory, for types */
	...
	if (actual == 0) {
		...
		kr = vm_map_pageable(ipc_kernel_map,
				     addr1, addr1 + vm_size_used,
				     TRUE);
		assert(kr == KERN_SUCCESS);

		kr = vm_map_pageable(ipc_kernel_map,
				     addr2, addr2 + vm_size_used,
				     TRUE);
		assert(kr == KERN_SUCCESS);
...
#if	MACH_OLD_VM_COPY
	vm_offset_t memory;	/* copied-in memory */
#else	/* MACH_OLD_VM_COPY */
	...
	if (kr == KERN_SUCCESS) {
...
#if	MACH_OLD_VM_COPY
		vm_offset_t copy = (vm_offset_t) *typesp;
#else	/* MACH_OLD_VM_COPY */
		...
		if (kr != KERN_SUCCESS) {
			...
			kmem_free(
				...
				(vm_offset_t) *typesp,
				(vm_size_t)
				round_page((ipc_entry_num_t) *typesCnt *
						sizeof(mach_port_type_t)));
			kmem_free(
				ipc_soft_map,
				(vm_offset_t) *namesp,
				(vm_size_t)
				round_page((ipc_entry_num_t) *namesCnt *
						sizeof(mach_port_t)));
#else	/* MACH_OLD_VM_COPY */
			...
			(void) vm_deallocate(ipc_soft_map, copy, size);
#endif	/* MACH_OLD_VM_COPY */
