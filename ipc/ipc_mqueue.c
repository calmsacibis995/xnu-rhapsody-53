	...
	ipc_port_t	port)
{
	...
	for (kmsg = ipc_kmsg_queue_first(oldq);
		...
		if (kmsg->ikm_header.msgh_bits & MACH_MSGH_BITS_OLD_FORMAT) {
			...
			if (!(th->ith_rcv_option & MACH_RCV_OLD_FORMAT)) {
			    th->ith_state = MACH_RCV_HEADER_ERROR;
			    th->ith_kmsg = kmsg;
			...
			if (th->ith_rcv_option & MACH_RCV_OLD_FORMAT) {
			    ...
			    /*
			     * As a special case, allow simple
			     * messages which consist of only a
			     * header to be received with the old
			     * messaging primatives.
			     */
			    th->ith_state = MACH_MSG_SUCCESS;
			}
			else
			if (kmsg->ikm_header.msgh_size > (th->ith_msize - 
				REQUESTED_TRAILER_SIZE(th->ith_rcv_option))) {
			    /*
			     * The receive buffer isn't large enough
			     */
			    th->ith_state = MACH_RCV_TOO_LARGE;

			    if (th->ith_rcv_option & MACH_RCV_LARGE) {
			    	th->ith_msize = kmsg->ikm_header.msgh_size;
			    	thread_go(th);
				continue;
			    }
			...
			if (msg_is_complex && th->ith_list != IKM_NULL &&
			    (th->ith_state =
				    	ipc_kmsg_check_scatter(
						kmsg, th->ith_rcv_option,
						&th->ith_list))
							!= MACH_MSG_SUCCESS) {
			    /*
			     * Check for type/VM errors.
			     */
			    if (th->ith_state == MACH_RCV_INVALID_TYPE ||
			    	(th->ith_state & MACH_MSG_VM_KERNEL) != 0) {
				thread_go(th);
				continue;
			    }

			    /*
			     * The scatter list isn't large enough.
			     */
			    if (th->ith_rcv_option & MACH_RCV_LARGE) {
			    	th->ith_msize = kmsg->ikm_header.msgh_size;
				thread_go(th);
	...
	void 			(*continuation)(void))
{
	...
	if (kmsg->ikm_header.msgh_bits & MACH_MSGH_BITS_OLD_FORMAT) {
		...
		if (!(receiver->ith_rcv_option & MACH_RCV_OLD_FORMAT)) {
		    receiver->ith_state = MACH_RCV_HEADER_ERROR;
		...
		if (kmsg->ikm_header.msgh_size <= receiver->ith_msize) {
		    ...
		    if (option & MACH_SEND_SWITCH)
			    thread_go_and_switch(continuation, receiver);
		    else
			    thread_go(receiver);
		    break;
		}
		...
		if (kmsg->ikm_header.msgh_size > (receiver->ith_msize -
			REQUESTED_TRAILER_SIZE(receiver->ith_rcv_option))) {
		    /*
		     * The receive buffer isn't large enough
		     */
		    receiver->ith_state = MACH_RCV_TOO_LARGE;

		    if (receiver->ith_rcv_option & MACH_RCV_LARGE) {
		    	receiver->ith_msize = kmsg->ikm_header.msgh_size;
			thread_go(receiver);
			continue;
		    }
		...
		if (msg_is_complex && receiver->ith_list != IKM_NULL &&
		    (receiver->ith_state =
				    ipc_kmsg_check_scatter(
				    	kmsg, receiver->ith_rcv_option,
					&receiver->ith_list))
							!= MACH_MSG_SUCCESS) {
		    /*
		     * Check for type/VM errors.
			...
			(receiver->ith_state & MACH_MSG_VM_KERNEL) != 0) {
			thread_go(receiver);
			continue;
		    }
...
mach_msg_return_t
ipc_mqueue_send_interrupt(kmsg)
	ipc_kmsg_t kmsg;
{
	ipc_port_t port;
	...
	assert(!(kmsg->ikm_header.msgh_bits &
			(MACH_MSGH_BITS_COMPLEX|MACH_MSGH_BITS_OLD_FORMAT)));
	...
	ipc_kmsg_t		*list)
{
	...
	for (;;) {
		...
		if (kmsg != IKM_NULL) {
			...
			/*
			 * Old format message. (typed data)  Verify
			 * that one of the old messaging primatives
			 * is being used for the receive operation.
			 */
			if (!(option & MACH_RCV_OLD_FORMAT)) {
			    	self->ith_state = MACH_RCV_HEADER_ERROR;
			...
			self->ith_state = MACH_MSG_SUCCESS;
		    	...
		    	/*
			 * New format message. (untyped data)  Verify
			 * that one of the new messaging primatives
			 * is being used for the receive operation.
			 */
			if (option & MACH_RCV_OLD_FORMAT) {
			    if ((kmsg->ikm_header.msgh_bits &
			...
			if (kmsg->ikm_header.msgh_size > (max_size -
			    ...
			    self->ith_state = MACH_RCV_TOO_LARGE;
			}
			else
			if ((kmsg->ikm_header.msgh_bits &
					MACH_MSGH_BITS_COMPLEX) &&
			     ...
			     * Check for type/VM errors.
			     */
			    if (self->ith_state == MACH_RCV_INVALID_TYPE ||
				(self->ith_state & MACH_MSG_VM_KERNEL) != 0) {
				imq_unlock(mqueue);
		...
		self->ith_rcv_option = option;
		self->ith_list = list ? *list : IKM_NULL;
...
return_kmsg:
		...
		switch (self->ith_state) {
			 ...
			 * the gather list.
			 */
		    	if (!(self->ith_rcv_option &
				(MACH_RCV_LARGE|MACH_RCV_OLD_FORMAT)))
			    goto return_kmsg;
			...
			/*
			 * There was a message was queued on the
			 * port, but its format was incompatible
			 * with the receive operation we were
			 * performing.
			 */
			goto return_kmsg;
