...
/*
	...
	Change History (most recent first):
	   ...
	   <Rhap1>	 3/26/97	djb		Added PrintNode for debugging.
...
OSStatus	CheckNode	(BTreeControlBlockPtr	 btreePtr, NodeDescPtr	 node )
{
...
#if DEBUG_BUILD
	KeyPtr				thisKey, lastKey = NULL;
	KeyCompareProcPtr	compareProc = NULL;
#endif
	...
	if ( (node->type < kLeafNode) || (node->type > kMapNode) )
		return fsBTInvalidNodeErr;
	...
	//€€ can we calculate a more accurate minimum record size?
	...
	index = node->numRecords;					// begin with free space offset
	offset = nodeSize - (index << 1);				// init offset to max free offset+2
	
#if DEBUG_BUILD
	if ( btreePtr->attributes & kBTVariableIndexKeysMask )
	{
		compareProc = btreePtr->keyCompareProc;
		lastKey = NULL;
	}
#endif
...
#if DEBUG_BUILD	
		if ( btreePtr->attributes & kBTVariableIndexKeysMask	&&
			 index < node->numRecords							&&
			 (node->type == kLeafNode || node->type == kIndexNode) )
		{
			thisKey = (KeyPtr) GetRecordAddress (btreePtr, node, index );
	
			if ( (lastKey != NULL) && (compareProc(thisKey, lastKey) >= 0) )
			{
				Panic ( "\p CheckNode: keys out of order! ");
				return fsBTInvalidNodeErr;
			}
