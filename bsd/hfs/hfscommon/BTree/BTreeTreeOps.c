						 ...
						 UInt16					*returnIndex )
{
	...
	while (true)
	{
		...
		if ( ((BTNodeDescriptor*)nodeRec.buffer)->type == kLeafNode)
		{
						 ...
						 UInt32						*insertNode )
{
	...
	PanicIf ((level == 1) && (((NodeDescPtr)targetNode->buffer)->type != kLeafNode), "\P InsertLevel: non-leaf at level 1! ");
#endif
	leftNode.buffer = nil;
	...
	if ( insertParent || updateParent )
	{
		...
		if (DEBUG_BUILD && level > 1)
			PanicIf ( ((NodeDescPtr)parentNode.buffer)->type != kIndexNode, "\P InsertLevel: parent node not an index node! ");
#endif
		////////////////////////// Update Parent Index //////////////////////////////
	...
	err = UpdateNode (btreePtr, &leftNode);		// all done with left sibling
	M_ExitOnError (err);
								 ...
								 UInt16						 level )
{
	...
	if ( targetNodePtr->numRecords == 0 )	// did we delete the last record?
	{
		...
		else if ( targetNodePtr->type == kLeafNode )		// update firstLeafNode
		{
		...
		else if ( targetNodePtr->type == kLeafNode )		// update lastLeafNode
		{
							 	 ...
							 	 BlockDescriptor			*blockPtr )
{
	...
	while (true)
	{
		...
		if ( ((NodeDescPtr)blockPtr->buffer)->type == kLeafNode)
								 ...
								 UInt16						*recsRotated )
{
	...
	keyLength = GetKeyLength(btreePtr, keyPtr, (rightNode->type == kLeafNode));
								 ...
								 UInt16						*recsRotated )
{
	...
	if ( (right->height == 1) && (right->type != kLeafNode) )
	...
	if ( (left->type == kLeafNode) && (left->bLink == 0) )
	{
								 ...
								 NodeDescPtr			 rightNode )
{
	...
	((NodeDescPtr)rootNode.buffer)->type	= kIndexNode;
