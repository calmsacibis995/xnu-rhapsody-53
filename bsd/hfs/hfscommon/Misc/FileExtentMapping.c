...
/*
	...
	Change History (most recent first):
	  <Rhap>	 6/30/98	djb		Add functions NodesAreContiguous and ExtentsAreIntegral (for radar #2249539).
	  <Rhap>	 6/23/98	djb		Changed DeallocFile to DeleteFile which now deletes the catalog record.
	  								Fixed UpdateExtentRecord to pass correct fcb to Btree routines. Fixed
	  								hfs+ bug in CreateExtentRecord (double dereference).
	  <Rhap>	 5/20/98	djb		In ExtendFileC don't lie about the peof! (radar #2230094).
	  <Rhap>	 4/17/98	djb		Add VCB locking.
	  <Rhap>	  4/2/98	djb		Switch over to real BTree interface (no more BTreeWrapper.c).
		...
		if (err == noErr) {
			...
			if (bytesThisExtent > bytesToAdd) {
				if (TARGET_OS_RHAPSODY)
					fcb->fcbPLen += bytesThisExtent;	/* don't lie about the peof in Rhapsody */
				else
					fcb->fcbPLen += bytesToAdd;			/* Mac OS semantics support lying (peof <= sum of extents) */
				bytesToAdd = 0;
			}
			else {
				bytesToAdd -= bytesThisExtent;
...
#else
        return &(((struct vfsFCB *)(fcb))->fcb_extFCB);
#endif
}
	...
	const HFSPlusExtentRecord	extents)
{
	...
	if ( isHFSPlus ) {
		extendedFCB = GetExtendedFCB(fcb);
		BlockMoveData(extents, extendedFCB->extents, sizeof(HFSPlusExtentRecord));
	}
	else {
	...
	UInt32		nodeSize)
{
	...
	{
		result = FindExtentRecord(vcb, kDataForkType, fcb->fcbFlNm, startBlock, FALSE, &key, extents, &hint);
		if (result) break;
...
void AdjustEOF(FCB *sourceFCB)
{
	...
	FileReference		refnum;			//	Used for iterating over FCBs
	...
	for (refnum=kFirstFileRefnum; refnum<maxRefnum; refnum+=lengthFCB) {
			...
			(destFCB->fcbFlags & fcbResourceMask) == resourceFlag)
		{
			...
			if (isHFSPlus) {
				...
				BlockMoveData(srcExtendedFCB->extents, destExtendedFCB->extents, sizeof(HFSPlusExtentRecord));
			}
			else {
...
static OSErr AllocateLargeFile(
	...
	UInt32					*dataBlocks,
	UInt32					*rsrcBlocks,
	HFSPlusExtentRecord		dataExtents,
	HFSPlusExtentRecord		rsrcExtents)
{
	OSErr				err;
	int					i;
	UInt32				blockSize;
	...
	ClearMemory(dataExtents, sizeof(HFSPlusExtentRecord));
	ClearMemory(rsrcExtents, sizeof(HFSPlusExtentRecord));
	...
	*dataBlocks = blocksToAllocate;
	...
	if (blocksToAllocate) {
		for (i=0; i<kHFSPlusExtentDensity && blocksToAllocate; i++) {
...
OSErr CreateLargeFile(CreateLargeFileParam *pb)
{
	...
	//	Allocate the space
	//
	err = AllocateHFSPlusFile(pb, vcb, &dataBlocks, &rsrcBlocks, dataExtents, rsrcExtents);
	ReturnIfError( err );
	...
	//
	err = CreateCatalogNode(vcb, fileInfo.id, name, kHFSFileRecord, &fileID, &hint);
	ReturnIfError( err );
	...
	//
	recordData.hfsPlusFile.dataFork.logicalSize = pb->dataLogicalEOF;
	recordData.hfsPlusFile.dataFork.totalBlocks = dataBlocks;
	BlockMoveData(dataExtents, recordData.hfsPlusFile.dataFork.extents, sizeof(HFSPlusExtentRecord));

	recordData.hfsPlusFile.resourceFork.logicalSize = pb->rsrcLogicalEOF;
	recordData.hfsPlusFile.resourceFork.totalBlocks = rsrcBlocks;
	BlockMoveData(rsrcExtents, recordData.hfsPlusFile.resourceFork.extents, sizeof(HFSPlusExtentRecord));
	
	//
	//	Write the updated record back out
	//
	err = ReplaceBTreeRecord(vcb->catalogRefNum, &recordKey, hint, &recordData, sizeof(HFSPlusCatalogFile), &hint);
