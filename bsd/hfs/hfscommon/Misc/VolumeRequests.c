...
/*
	...
	Change History (most recent first):
	  <Rhap>	 7/28/98	djb		GetDiskBlocks is now implemented in MacOSStubs.c (radar #2258148).
	  <Rhap>	  4/3/98	djb		Conditionally remove FSVars reference from GetVolumeNameFromCatalog.
...
#if TARGET_OS_MAC
OSErr	FindFileControlBlock( File *index, Ptr *fcbsH );
...
OSErr	MountVolume( VolumeParam *volume, Boolean forceHFSWrapperToMount )
{
	...
	if ( err == noErr )
	{
		...
		pureHFSPlusVolume	= ((HFSPlusVolumeHeader *)blockBuffer)->signature == kHFSPlusSigWord;
		
		if ( 	( ((HFSMasterDirectoryBlock *)blockBuffer)->drEmbedSigWord == kHFSPlusSigWord	//	Wrapped HFS+ volume
...
OSErr	MountHFSPlusVolume( short driveNumber, IOParam* lowMemParamBlock, OSErr err, Boolean pureHFSPlusVolume )
{
	...
	{
		mdb = (HFSMasterDirectoryBlock *) lowMemParamBlock->ioBuffer;
	...
	volumeHeader = (HFSPlusVolumeHeader *) lowMemParamBlock->ioBuffer;
...
mv_Start:
	...
	*flagsP = *pbFlagsP & kVolumeHardwareLockMask;
	...
	if ( IsARamDiskDriver() )
	{
		vcb->vcbAtrb |= kVolumeNoCacheRequiredMask;								//	yes:Êmark VCB as a RAM Disk
	}
	...
	BlockMoveData( volumeHeader->extentsFile.extents, extendedFCB->extents, sizeof(HFSPlusExtentRecord) );
	...
	extendedFCB = ParallelFCBFromRefnum( fcbIndexP );
	BlockMoveData( volumeHeader->catalogFile.extents, extendedFCB->extents, sizeof(HFSPlusExtentRecord) );
	...
	extendedFCB = ParallelFCBFromRefnum( fcbIndexP );
	BlockMoveData( volumeHeader->allocationFile.extents, extendedFCB->extents, sizeof(HFSPlusExtentRecord) );
	...
	if (volumeHeader->attributesFile.extents[0].blockCount != 0)
	{
		...
		extendedFCB = ParallelFCBFromRefnum( fcbIndexP );
		BlockMoveData( volumeHeader->attributesFile.extents, extendedFCB->extents, sizeof(HFSPlusExtentRecord) );
	}
...
OSErr	MountHFSVolume( short driveNumber, IOParam* lowMemParamBlock, OSErr err )
{
	...
	ioRefNum = lowMemParamBlock->ioRefNum;
	mdb = (HFSMasterDirectoryBlock *) lowMemParamBlock->ioBuffer;			//	¥¥ figure out how we use VolumeHeader & MasterDirectoryBlock, size of structures, etc.	
...
mv_Start:
	...
	*flagsP = *mdbFlagsP & kVolumeHardwareLockMask;
	...
	vcb->extentsRefNum		= fcbIndexP;										//	Set Extents B*-Tree file RefNum
	BlockMoveData( mdb->drXTExtRec, fcb->fcbExtRec, sizeof(HFSExtentRecord) );
	...
	vcb->catalogRefNum		= fcbIndexP;										//	Set Catalog B*-Tree file RefNum
	BlockMoveData( mdb->drCTExtRec, fcb->fcbExtRec, sizeof(HFSExtentRecord) );
...
OSErr	CheckVolumeConsistency( ExtendedVCB *vcb )
{
	...
	if ( vcb->vcbSigWord == kHFSSigWord)					//	and is an HFS disk
#else 	/* TARGET_OS_RHAPSODY */
	...
	{
		if ( vcb->vcbAtrb & kHFSBootVolumeInconsistentMask )		//	was marked inconsistant
		{
			err = MountCheck( vcb, &consistencyStatus );
			
			if ( err == noErr )
				vcb->vcbAtrb &= ~kHFSVolumeSoftwareLockMask;		//	clear the bit
		}
#if TARGET_OS_MAC
		if ( LMGetBootDrive() == vcb->vcbDrvNum )				//	this is the boot volume
			vcb->vcbAtrb |= kHFSBootVolumeInconsistentMask;
#endif 	/* TARGET_OS_MAC */
	}
...
OSErr	FlushVolumeControlBlock( ExtendedVCB *vcb )
{
	...
	if ( vcb->vcbSigWord == kHFSPlusSigWord )
	{
		...
		#if( DEBUG_BUILD )
			if ( volumeHeader->signature !=	vcb->vcbSigWord )
				DebugStr("\p Bad VolumeHeader, signature does not match vcb" );
		#endif
		ReturnIfError( err );
	...
	//¥¥	volumeName
		...
		BlockMoveData( extendedFCB->extents, volumeHeader->extentsFile.extents, sizeof(HFSPlusExtentRecord) );
		fcb = GetFileControlBlock( index );
		...
		extendedFCB = GetParallelFCB( index );
		BlockMoveData( extendedFCB->extents, volumeHeader->catalogFile.extents, sizeof(HFSPlusExtentRecord) );
		fcb = GetFileControlBlock( index );
		...
		extendedFCB = GetParallelFCB( index );
		BlockMoveData( extendedFCB->extents, volumeHeader->allocationFile.extents, sizeof(HFSPlusExtentRecord) );
		fcb = GetFileControlBlock( index );
		...
		{
			extendedFCB = GetParallelFCB( index );
			BlockMoveData( extendedFCB->extents, volumeHeader->attributesFile.extents, sizeof(HFSPlusExtentRecord) );
			fcb = GetFileControlBlock( index );
		...
		MarkVCBClean( vcb );
#else
		// This does now what above does also, so lets keep it all together in a single place
...
ExtendedFCB* GetParallelFCB(FileReference fref)
{
#if TARGET_OS_MAC
...
#else
	return &VTOH(fref)->h_xfcb->fcb_extFCB;
#endif /* TARGET_OS_MAC */
}
...
OSErr	FlushAlternateVolumeControlBlock( ExtendedVCB *vcb, Boolean isHFSPlus )
{
...
#else
                err = GetBlock_glue( gbDefault, (vcb->hfsPlusIOPosOffset / 512) + kIDSectorOffset, (Ptr*)&volumeHeader, kNoFileReference, vcb );	//	VH is always 1st sector of HFS Plus partition
#endif
		ReturnIfError( err );
...
#else
                err = GetBlock_glue( gbDefault, alternateBlockLocation, (Ptr*)&alternateVolumeHeader, kNoFileReference, vcb );	//	Alt VH is always 2nd to last sector of HFS Plus partition
#endif
		if ( err == noErr )
		{
			//--	Copy the VolumeHeader to the alternate location
			BlockMoveData( volumeHeader, alternateVolumeHeader, sizeof(HFSPlusVolumeHeader) );
	...
	else																		//	Flush the MasterDirectoryBlock
	{
		...
		err = GetDiskBlocks( vcb, &numBlocks );
		ReturnIfError( err );
...
#else
                err = GetBlock_glue( gbDefault, kIDSectorOffset, (Ptr*)&mdb, kNoFileReference, vcb );	//	no Flags, MDB is always block
#endif
		ReturnIfError( err );
...
#else
                err = GetBlock_glue( gbDefault, alternateBlockLocation, (Ptr*)&alternateMDB, kNoFileReference, vcb );	//	no Flags, VH is always block #0, vcb->vcbBufAdr == CacheQueueHeader
#endif
		if ( err == noErr )
		{
			//--	Copy the MDB to the alternate location
			BlockMoveData( mdb, alternateMDB, sizeof(HFSMasterDirectoryBlock) );
...
//_______________________________________________________________________
//
//	Routine:	TrashAllFSCaches
//	Arguments:	ExtendedVCB		*vcb
//	Output:		OSErr			err
//
//	Function: 	Trash all file system related caches as well as the disk cache.
//				This allows applications such as Disk First Aid or Norton Utilities
//				to be able to repair disks without first unmounting the volume.
//				
//				All file system caches should also have a call to the cache
//				trashing routine here!
//_______________________________________________________________________
#if TARGET_OS_MAC
OSErr	TrashAllFSCaches( ExtendedVCB *vcb )
{
	OSErr			err;
	FSVarsRec*		fsVars	= (FSVarsRec*) LMGetFSMVars();
	
	InvalidateCatalogCache( vcb );				//	Calls TrashCatalogIterator 
	TrashCatalogNodeCache( );					//	Trash Catalog node cache, gCatalogFSSpec
	err = TrashCacheBlocks( 0 );				//	Trash BTree node cache
	TrashMRUCache( vcb->hintCachePtr );			//	Trash the heuristic hint cache
	ClearMemory( fsVars->gAttributesBuffer, fsVars->gAttributesBufferSize );	//	Clear the buffer used by MountCheck and CatSearch
	TrashVolumeDiskCache( vcb );				//	Trash the disk cache, calls TRASHVBLKSA7
...
#else
#pragma unused(vcb,driveNumber,ioRefNum,wdcbH)
#endif 	/* TARGET_OS_MAC */
}
...
#else
			vcb->vcbVN[0] = strlen(spec.name);
			BlockMoveData( spec.name, &vcb->vcbVN[1], sizeof(Str27)-1 );
#endif
