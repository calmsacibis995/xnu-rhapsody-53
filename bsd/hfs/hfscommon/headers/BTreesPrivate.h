...
/*
	...
	Change History (most recent first):
	   ...
	   <Rhap>	 8/10/98	djb		Removed unused BTreeIterator from BTreeControlBlock, fixed alignment.
...
#define		M_HeaderMapRecordSize(nodeSize)		(nodeSize - sizeof (HeaderRec) - 128 - 8)
...
#else
	UInt8						 reserved3[20];		// keep for alignment with old style fields
#endif
...
#define		MoveRecordsLeft(src,dst,bytes)			BlockMoveData((src),(dst),(bytes))

void		MoveRecordsRight		(UInt8 *				 src,
									 UInt8 *				 dst,
									 UInt16					 bytesToMove );

#define		MoveRecordsRight(src,dst,bytes)			BlockMoveData((src),(dst),(bytes))
