	...
	aurp_state_t *state;
{
	...
	state->rcv_tmo = (void *)atalk_timeout(
		AURPsndRIReq, state, AURP_RetryInterval*HZ);
}
	...
	aurp_state_t *state;
{
	...
	if (state->rsp_m == 0) {
		...
		if ((m = (gbuf_t *)gbuf_alloc(msize+AURP_MaxPktSize, PRI_MED)) == 0) {
			state->snd_tmo = (void *)
				atalk_timeout(AURPsndRIRsp, state, AURP_RetryInterval*HZ);
			return;
		}
		gbuf_wset(m,msize);
		state->rsp_m = m;
	...
	aurp_state_t *state;
{
	...
	/* start the retry timer */
	state->snd_tmo = (void *)
		atalk_timeout(AURPsndRIUpd, state, AURP_RetryInterval*HZ);
	...
	gbuf_t *m;
{
	...
	dPrintf(D_M_AURP, D_L_INFO, ("AURPrcvRIRsp: len=%d\n", gbuf_len(m)));

	/* cancel the retry timer */
	atalk_untimeout(AURPsndRIReq, state, state->rcv_tmo);
	state->rcv_tmo = 0;
	...
	gbuf_t *m;
{
	...
	dPrintf(D_M_AURP, D_L_INFO, ("AURPrcvRIUpd: len=%d\n", gbuf_len(m)));
	...
	gbuf_t *m;
{
	...
	if (((snd_state == AURPSTATE_WaitingForRIAck1) ||
		...
		if (snd_state == AURPSTATE_WaitingForRIAck1) {
			...
			atalk_untimeout(AURPsndRIRsp, state, state->snd_tmo);
			dat_m = state->rsp_m;
			state->rsp_m = 0;
			flag = 1;
		} else {
			/* ack from the tunnel peer to our RI update */
			atalk_untimeout(AURPsndRIUpd, state, state->snd_tmo);
			dat_m = state->upd_m;
			state->upd_m = 0;
			flag = 2;
		}
	...
	gbuf_t *m;
{
	...
	while (tuples_cnt--) {
		...
		if ((curr_rt = RT_lookup(new_rt.NetStop)) != 0) { /* found? */
	...
	gbuf_t *m;
{
	...
	while (gbuf_len(m) > 0) {
	  ...
	  case AURPEV_NetAdded:
		...
		if ((old_rt = RT_lookup(new_rt.NetStop)) != 0) { /* found? */
...
l_add:		if ((new_rt.NetDist < 16) && (new_rt.NetDist != NOTIFY_N_DIST)) {
	  ...
	  case AURPEV_NetRouteChange:
		gbuf_rinc(m,ev_len);
l_delete:	if ((old_rt = RT_lookup(new_rt.NetStop)) != 0) { /* found? */
			if (old_rt->NetPort == net_port) {
