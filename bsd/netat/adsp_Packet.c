    ...
    MATCHPtr m;
{
	...
	if (match & M_ADDR) {	/* Match on Address */
		...
		return 0;
	    if ((NET_VALUE(sp->remoteAddress.a.net) && NET_VALUE(addr.a.net)) &&
		(NET_VALUE(sp->remoteAddress.a.net) != NET_VALUE(addr.a.net)))
		return 0;
	     ...
	     */
 		...
 		(addr.a.node == ddpcfg.node_addr.node) && 
 		((NET_VALUE(addr.a.net) == 0) || 
	...
	if (match & M_FILTER) {	/* Check address filter? */
		...
		AddrUnion	addr;
		addr = m->addr;	/* Make local copy for efficiency */
		if ((NET_VALUE(opb->u.openParams.filterAddress.net) && 
		     NET_VALUE(addr.a.net) &&
		     NET_VALUE(opb->u.openParams.filterAddress.net) != NET_VALUE(addr.a.net)) ||
    ...
    unsigned char dsoc;
{
	...
	while (n = (struct adspcmd *)p->qLink) /* Hunt down list of listens */
	{
	    ...
	    if (((NET_VALUE(n->u.openParams.filterAddress.net) == 0) ||
		 (NET_VALUE(addr.a.net) == 0) ||
		 (NET_VALUE(n->u.openParams.filterAddress.net) == NET_VALUE(addr.a.net))) &&
