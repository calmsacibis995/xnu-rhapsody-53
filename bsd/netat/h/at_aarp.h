...
#define	AMT_LOOK(at, at_addr, elapp) {							\
	register n; 								\
	at = &et_aarp_amt[elapp->pat_id][AMT_HASH(at_addr) * AMT_BSIZ];	 		\
	for (n = 0 ; ; at++) {					                \
...
#define	NEW_AMT(at, at_addr, elapp) {							\
	...
	myat = at = &et_aarp_amt[elapp->pat_id][AMT_HASH(at_addr) * AMT_BSIZ];			\
	for (n = 0 ; ; at++) {					                \
	    if (at->last_time == 0)             				\
	        break;  							\
	    if (++n >= AMT_BSIZ) {					        \
...
#define	AARP_NET_MCAST(p, elapp)						\
 	(NET_EQUAL((p)->dst_net, elapp->elap_if.ifThisNode.atalk_net)		\
...
extern	struct	etalk_addr	etalk_multicast_addr;
extern	struct	etalk_addr	ttalk_multicast_addr;
extern	struct	etalk_addr	zone_multicast_addr;
