...
extern void *atalk_timeout(void (*func)(), void *arg, int ticks);
extern void atalk_untimeout(void (*func)(), void *arg, void *trb);
	...
	gref_t *gref;
{
	...
	untimeout(asp_send_tickle, (void *)scb); /* added for 2225395 */
     ...
     int state;
{
  return ((state==ASPSTATE_Close)? "Close":
	  ...
	  (state==ASPSTATE_WaitingForCfgAck)? "CfgAck": "unknown");
}
...
return (
	...
	(aspIOC==ASPIOC_EnableSelect)? "EnableSbelect":
...
l_err:	gbuf_freem(m);
	...
	case MSG_PROTO:
		...
		dPrintf(D_M_ASP, D_L_INFO,
			("asp_wput: MSG_PROTO %s\n", aspCmdStr(Primitive)));

		switch (Primitive) {

/*#### LD 10/15/97:
 * We reuse the request mbuf to layout the atpBDS stucture, and if the
 * mbuf size is 128, this corrupts the next mbfu header physically
 * contiguous. To avoid this, a new mbuf could be allocated for the m_next
 * of the request message, and the structure allocated in a cluster.
 * However, the reuse of the mbuf is more efficient if we know for sure
 * that the MSIZE is 256 or bigger. If for any reason
 * MSIZE needs to be changed back to 128, the code reusing the mbuf
 * needs to be changed.
 */

#if (MSIZE == 128)
	panic("AppleTalk/ASP mbuf reuse not permited MSIZE too small");
#endif
		...
		case ASPFUNC_CmdReply:
			...
			*(long *)atp->user_bytes = (long)result;
			...
			dPrintf(D_M_ASP, D_L_INFO,
			("asp_wput: ATP CmdReplyReq, loc=%d, state=%s, msgsize = %d, result = %d, tid = %d\n",
				scb->loc_addr.socket, aspStateStr(scb->state), 
				(mx ? gbuf_msgsize(mx) : 0), result, tid));
			atp_send_rsp(gref, mdata, TRUE);
			return 0;
		}
	...
	case MSG_IOCTL:
		...
		dPrintf(D_M_ASP_LOW, D_L_INFO,
			("asp_wput: MSG_IOCTL %s, loc=%d, state=%s\n", 
	...
	int *errp;
{
	 ...
	 */
	mproto = gbuf_alloc_wait(ctlbuf.len, TRUE);
	if (!mproto)
		return -1;

	gbuf_set_type(mproto, MSG_PROTO);
	 ...
	 */
	if (datbuf.len) {
		mdata = gbuf_alloc_wait(datbuf.len+len, TRUE);
		if (!mdata)
			return -1;
