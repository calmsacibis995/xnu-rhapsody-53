...
/*
 *
 *	@(#)if_blue.c	1.1 (Rhapsody) 6/10/97
 ...
 * Incoming packets are routed into Rhapsody as well as to the requesting
 *  interface.
...
#define IFA2IN(ifa) \
	...
	ifb->blue_pid = current_thread()->task->proc->p_pid;
	ifb->ifb_so = so;
	ifp->if_Y = (void *)ifb;
	return(0);
}
...
{	register struct ifnet_blue *ifb;
	...
	if ((rv = BlueFilter_check(&m1, ifb)) == -1)
		return(m1);	/* Not for BB, Rhapsody will want to see it. */
	m = m1;
	if (rv == 0)		/* It's for both - dup the packet */
	{	m0 = m_dup(m, M_DONTWAIT);
		if (m0 == NULL)
		{	blue_if->no_bufs1++;
			return(m); /* Give it to Rhapsody */
		}
	} else
...
{	extern struct ifnet_blue *blue_if;
	...
	{	/* If we're the guy holding the Y-adapter, clean it up */
		if (blue_if->blue_pid == current_thread()->task->proc->p_pid)
		{	if (np->nd_if)
			...
			ndrv_flushq(&blueq);
			if (np->nd_laddr)
			{	m_freem(dtom(np->nd_laddr));
				np->nd_laddr = 0;
			}
		}
	}
	remque((queue_t)np);
	_FREE((caddr_t)np, M_PCB);
}
