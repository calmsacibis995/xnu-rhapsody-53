 ...
 *
 * History:
 *	 2/18/98	Pat Dirks, Apple Computer		New Today
 *
 */
...
#define ATTR_VOL_VALIDMASK			0x80007FFF
...
struct fullcommonattrblock {
	dev_t					device;
	fsid_t					fs;
	fsobj_type_t			fsobjtype;
	fsobj_tag_t				fsobjtag;
	fsobj_id_t				fsobjid;
	fsobj_id_t				fsparid;
	...
	gid_t					group;
	mode_t					mode;
	u_int32_t				namedattr_count;
	attrreference_t			namedattr_list;
	u_int32_t				flags;
};
...
struct fullattrblock {
	u_int32_t attrblocklength;
	struct fullcommonattrblock c;
	union {
		struct fullvolumeattrblock v;
		struct fulldirattrblock d;
		struct fullfileattrblock f;
		struct fullforkattrblock fork;
	} u;
};
