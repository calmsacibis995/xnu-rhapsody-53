...
/*
 * Time sources, in ticks and milliseconds.  These work in both
 * the kernel and user environments, as long as the driverkit is
 * linked in.  The tick time is on the same order as the old vertical blanking
 * counter, but is derived from the nanosecond timer timebase.
 */
#if defined(_KERNEL)
extern void IOGetTimestamp(ns_time_t *nsp);	// Can not include generalFuncs.h
						// due to PS conflicts with
						// Adobe code.
...
EvTickTimeValue()
{
	ns_time_t	ns_time;
	unsigned	tick;
	IOGetTimestamp(&ns_time);
	// Convert nanosec => ticks and return
	tick = EV_NS_TO_TICK(ns_time);
	if ( tick == 0 )
...
EvMsecTimeValue()
{
	...
	IOGetTimestamp(&ns_time);
	// Convert nanosec => millisec and return
	return (unsigned)(ns_time /= 1000000ULL);
}
#endif /* _KERNEL */
/* Pressure Constants */
...
/*
 * Get and Set parameter calls for the Event driver and associated filters.
 */

#define EV_PREFIX	"Ev_"	/* All EV calls start with this string */

/* _NXEvSetParameterInt() calls */
#define EVIOLLPE	"Ev_LLPostEvent"
#define EVIOPTRLLPE	"Ev_PointerLLPostEvent"
/* EventData packed as an array of ints. */
typedef union
{
    NXEventData	data;
    unsigned idata[(sizeof(NXEventData)+sizeof(unsigned)-1)/sizeof(unsigned)];
} _NX_packed_event_t;
...
#define EVIOLLPE_SIZE	(EVIOLLPE_DATA2 + 1)

#define EVIOSM	"Ev_MousePosition"		
typedef enum {
	EVIOSM_LOC_X,
	EVIOSM_LOC_Y
} evioSMIndices;
#define EVIOSM_SIZE	(EVIOSM_LOC_Y + 1)

#define EVIOST	"Ev_StartCursor"
#define EVIOST_SIZE	1

#define EVIOSETSCREEN	"Ev_SetScreen"
typedef enum {
	EVIOSETSCREEN_TOTALSCREENS,
	EVIOSETSCREEN_INDEX,
	EVIOSETSCREEN_SHMEMSIZE,
	EVIOSETSCREEN_MINX,
	EVIOSETSCREEN_MAXX,
	EVIOSETSCREEN_MINY,
	EVIOSETSCREEN_MAXY
}evioSETSCREENIndices;
#define EVIOSETSCREEN_SIZE	(EVIOSETSCREEN_MAXY + 1)


/* _NXEvGetParameterInt() calls */
#define EVIOEVNUM		"Ev_ButtonEventNums"	/* Mouse driver */
typedef enum {
	EVIOEVNUM_LEFT,
	EVIOEVNUM_RIGHT
} evioEVNUMIndices;
#define EVIOEVNUM_SIZE	(EVIOEVNUM_RIGHT + 1)

#define EVIOSHMEMSIZE	"Ev_ShmemSize"
#define EVIOSHMEMSIZE_SIZE	1

typedef char EVString[64];
typedef char EVParameterName[64];
typedef int EVIntParameter[64];
typedef char EVCharParameter[4096];
typedef unsigned int EVObjectNumber;
