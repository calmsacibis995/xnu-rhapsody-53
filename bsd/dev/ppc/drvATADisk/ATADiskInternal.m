 ...
 *
 * ATADiskInternal.m - internal IDE/ATA disk methods. 
 *
 * HISTORY 
 *
...
@implementation ATADisk(Internal)
 ...
 * for a given instance of ATADisk; initResources must have been called
...
- (BOOL)ideDiskInit:(unsigned int)diskUnit target:(unsigned int)unit
{
    ...
    if (_ideInfo.type == 0)	{
    //IOLog("ATADisk: Bogus info for disk %d target %d.\n", diskUnit, unit);
	return NO;
    }
    ...
    if ([_cntrlr getMultiSectorValue:unit] > 0) 
    {
        _ideReadCommandPIO  = IDE_READ_MULTIPLE;
        _ideWriteCommandPIO = IDE_WRITE_MULTIPLE;
    } 
    else 
    {
	_ideReadCommandPIO = IDE_READ;
	_ideWriteCommandPIO = IDE_WRITE;
    }
	...
	IOLog("%s: using PIO transfers.\n", [self name]);
        ...
        _ideWriteCommand = _ideWriteCommandPIO;
    }

    _ideControllerType = [_cntrlr getControllerType];
...
- (IOReturn) ideRwCommon:(ideBuf_t *)ideBuf
{
    ...
    {
	ioCommand = readFlag ? _ideReadCommand : _ideWriteCommand;
    }
...
volatile void ideThread(ATADisk *idisk)
{
    ideBuf_t *ideBuf;
    queue_head_t *q;
