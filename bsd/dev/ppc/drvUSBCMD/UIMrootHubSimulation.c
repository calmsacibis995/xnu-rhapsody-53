...
/*
	...
	Change History (most recent first):
	  ...
	  <USB4>	 8/25/98	BT		Isoc name changes
	  <USB3>	 8/12/98	BT		Allow more than 1 control at a time.
	  <USB2>	 6/30/98	BT		Tidy up
	  <USB1>	 6/30/98	BT		first checked in
*/


/*

	// UIMrootHubSimulation is USL's rootHubSimulation moved across 
...
// Need some way of talking to the outside here
#define USBExpertStatus(a,b, c)

// This isn't used at the moment
#define UIM_Bus 0

// Put these in globals struct????
typedef struct{
	UInt8 *buf;
	UInt32 bufLen;
...
//naga typedef UInt32 *QHdrPtr;   //naga temporary
static QHdrPtr simCallbackQueue;
...
#define HostToUSBWord Swap16
#define USBToHostWord Swap16


#if 0
	if(simCallbackQueue == nil)
	{
		if( (PBQueueCreate(&simCallbackQueue) != noErr) ||
			(PBQueueInit(simCallbackQueue) != noErr) )
		{
			/* Panic */
			USBExpertStatus(0,"\pUSL - failed to initialise root simulation queue", 0);
		}
	}


OSStatus UIMProcessDoneQueue(void)
{
USBPB *pb;

	if( (simCallbackQueue != nil) && (simCallbackQueue->qHead != nil) )
	{
		while(PBDequeueFirst(simCallbackQueue, (void *)&pb) == noErr)
		{
			uslInterruptPriority(-1);

			if(pb == nil)
			{
				USBExpertStatus(-1, "\pNil pb", 0);
			}
			else
			{
...
static void **UIM, *UIM1;
	checkUIMMoved(&UIM, &UIM1, (void *)&UIMPlugIn->uimProcessDoneQueueProc);
#endif

	if(UIMPlugIn != nil)
	{
		return(*UIMPlugIn->uimProcessDoneQueueProc)();
	}
	return(-1);
#else
	return(noErr);
...
static void simIntPktHandler(long refCon, OSStatus status, short shortfall)
{
USBPB *pb;
	...
	uslInterruptPriority(+1);
	PBEnqueueLast((void *)pb, simCallbackQueue);
}

static void simPktHandler(long refCon, OSStatus status, short shortfall)
{
USBPB *pb;
...
{
struct uslRootHubDescriptor 	descr; //naga struct added
int bytesOfFlags, copy, length;

	if(packet->word[wValue] == 0)
	{
	
		if(OHCIGetRootDescriptor(UIM_Bus, &descr) != noErr)
		{
			return(true);
		}
...
OSStatus err = -1;
	...
	switch(feature)
	{
		case kUSBHubPortConnectionChangeFeature :
			err =OHCIRootHubResetChangeConnection(UIM_Bus, port);
		break;
		
		case kUSBHubPortSuspendChangeFeature :
			err =OHCIRootHubResetSuspendChange(UIM_Bus, port);
		break;
		
		case kUSBHubPortEnablenFeature :
			err =OHCIRootHubPortEnable(UIM_Bus, port, false);
		break;
		
		case kUSBHubPortPowerFeature :
			err =OHCIRootHubPortPower(UIM_Bus, port, false);
			// Now need to check if all ports are switched off and 
			// gang off if in gang mode
		break;

		case kUSBHubPortResetChangeFeature :
			err =OHCIRootHubResetResetChange(UIM_Bus, port);
		break;

		case kUSBHubPortEnableChangeFeature :
			err =OHCIRootHubResetEnableChange(UIM_Bus, port);
		break;
		
		case kUSBHubPortOverCurrentChangeFeature :
			err =OHCIRootHubResetOverCurrentChange(UIM_Bus, port);
		break;
...
UInt32	rhStatus;
	...
	{
		if ((err = OHCIGetRhStatus(UIM_Bus, &rhStatus)) == noErr){
...
{
struct uslRootHubPortStatus stat;   //naga added struct
OSStatus err = -1;
UInt32 length;

	if(packet->word[wValue] == 0)
	{
		err = OHCIRootHubGetPortStatus(UIM_Bus, USBToHostWord(packet->word[wIndex]), &stat);
...
OSStatus err = -1;
	...
	switch(feature)
	{
		case kUSBHubPortSuspecdFeature :
			err = OHCIRootHubPortSuspend(UIM_Bus, port, true);
		break;
		
		case kUSBHubPortResetFeature :
			err = OHCIRootHubResetPort(UIM_Bus, port);
		break;
		
		case kUSBHubPortEnablenFeature :
			err = OHCIRootHubPortEnable(UIM_Bus, port, true);
		break;
		
		case kUSBHubPortPowerFeature :
			err = OHCIRootHubPortPower(UIM_Bus, port, true);
			if(err == noErr)
			{
				err = OHCIRootHubPower(UIM_Bus, true);
			}
		break;
...
OSStatus err = -1;
	...
	{	
		...
		// err = OHCIRootHubLPSChange(UIM_Bus, false);	// not implemented yet
	}
	...
	{	
		...
		// err = OHCIRootHubOCChange(UIM_Bus, false);	// not implemented yet
	}	
	return(err != noErr);
}
...
OSStatus err = -1;
	...
	{	
		...
		// err = OHCIRootHubLPSChange(UIM_Bus, true);	// not implemented yet
	}
	if(packet->word[wValue] == kUSBHubOverCurrentChangeFeature)
	{	
		USBExpertStatus(0,"\pRoot hub simulation - unimplemented Set Overcurrent Change Feature", 0);
		// err = OHCIRootHubOCChange(UIM_Bus, true);	// not implemented yet
	}	
	return(err != noErr);
}


void rootHubStatusChangeHandler(void)
{	/* Only called if the RHSC bit is set at int. */
UInt16 statusChanged;	/* only have 15 ports in OHCI */
UInt32 status, statusBit;
struct uslRootHubDescriptor 	descr;   //naga added struct
static numPorts=0;
int index, move;
struct uslRootHubPortStatus stat;  //naga added struct
intTrans last;
...
*/
	if(last.handler == nil)
	{
kprintf("rootHubStatusChangeHandler: last handler = nil\n");
		return;
	}
	if(CompareAndSwap(0, 1, &intLock))
	{
		...
		statusChanged = 0;
		statusBit = 1;
		if(OHCIGetRhStatus(UIM_Bus, &status) == noErr)
		{
			if( (status & kOHCIHcRhStatus_Change ) != 0)
			{
kprintf("Hub status change bit set to 1 \n");
...
kprintf("#ports=%d\n",numPorts);
				...
				else
				{
					...
					kprintf("USL - Root hub int simulation hosed. No ports, tell Barry");
				}
			}
			...
			for(index = 1; index <= numPorts; index++)
			{
				...
				if(OHCIRootHubGetPortStatus(UIM_Bus, index, &stat) == noErr)
				{
...
//naga		(*last.handler)(last.refCon, noErr, last.bufLen - move);
...
{
UInt32 intrStatus;
//naga	if(outstanding[0].handler != nil)
	if(outstanding[0].handler != nil)
	{
		if( (OHCIGetInterruptStatus(UIM_Bus, &intrStatus) == noErr) &&
		    ((intrStatus & kOHCIHcInterrupt_RHSC) != 0) )
		{
			OHCIClearInterruptStatus(UIM_Bus, kOHCIHcInterrupt_RHSC);
//kprintf("calling rootHubStatusChange handler\n");
...
static Boolean SimulateRootHub(setupPacket *packet, UInt8 *buf, UInt32 *bufLen)
{
...
//kprintf("SimulateRootHub:request=0x%x,type=0x%x\n",packet->byte[bRequest],type);
...
10000010B SYNCH_FRAME Zero Endpoint Two Frame Number#endif
		...
		switch(packet->byte[bRequest])
		{
			...
			case kUSBRqSetAddress:
				...
				{
					/* Set address */
					stall = OHCISetOurAddress(UIM_Bus, USBToHostWord(packet->word[wValue]));
				}
				else
				{
					stall = true;
...
typedef struct{
  ...
  UInt8 *buf;
  UInt32 bufLen;
  CallBackFuncPtr handler1;
  UInt32 refCon1;
  CallBackFuncPtr handler2;
  UInt32 refCon2;
  CallBackFuncPtr handler3;
  UInt32 refCon3;
  Boolean stall;
  int state;
	} RHTransaction;
	
static RHTransaction tr;
#define kMaxRHtr 16
static RHTransaction RHDone[kMaxRHtr];
static UInt32 inUse[kMaxRHtr];
...
static void cleanState(RHTransaction *tr)
{
	if( (tr->state != kDataRecevied) && (tr->state != kStatusDone) )
	{
		tr->buf = nil;
		tr->bufLen = 0;
		tr->handler2 = nil;
...
static RHTransaction *queueTR(RHTransaction *tr)
{
int i;

	for(i = 0; i< kMaxRHtr; i++)
	{
		if(CompareAndSwap(0, 1, &inUse[i]))
		{
			RHDone[i] = *tr;
			tr->state = kNone;
			cleanState(tr);
...
OSStatus UIMSimulateRootHubStages(
	...
	short						direction)
{
static directionReceived;
RHTransaction *trOut;

	if(endpointNumber != 0)
	{
		bufferRounding = 0;		// stop annoying warning
		return(-1);
	}

	if( (direction != kUSBIn) && (direction != kUSBOut))	// setup packet
	{	/* Setup packet over rides any previous state */
	
		if(tr.state != kNone)	// no queues we only cope with one (complete) control per frame.
		{
		...
		tr.handler1 = handler;
		tr.refCon1 = refcon;
		tr.state = kSetupReceived;
		
		directionReceived = (((*(UInt8 *)CBP) & 0x80) == 0)?kUSBOut:kUSBIn;


	}
	else
	{
		if(directionReceived != direction)	// status phase is reverse data 
		{
			tr.handler3 = handler;
			tr.refCon3 = refcon;
			trOut = queueTR(&tr);
			trOut->stall = SimulateRootHub(trOut->packet, trOut->buf, &trOut->bufLen);
			directionReceived = 0;
			trOut->state = kStatusDone;
			OHCISetFrameInterrupt();
			return(noErr);
		}
		else if(tr.state == kSetupReceived)
		{
			tr.buf = (void *)CBP;
			tr.bufLen = bufferSize;
			tr.handler2 = handler;
			tr.refCon2 = refcon;
			tr.state = kDataRecevied;
		}
		else 
		{
			tr.state = kNone;
...
static void rootHubDispatch(RHTransaction *tr)
{
CallBackFuncPtr handler;
UInt32 refCon;
UInt32 retBufLen;
	if(tr->state != kStatusDone)
	{
		return;
	}

	// need to sync to end of frame here
	if(tr->handler1 != nil)
		(*tr->handler1)(tr->refCon1, noErr, 0);
	if(tr->handler2 != nil)
		(*tr->handler2)(tr->refCon2, noErr, 0);
	handler = tr->handler3;
	refCon = tr->refCon3;
	retBufLen = tr->bufLen;
	tr->state = kNone;
	...
	if(handler != nil)
	{
		(*handler)(refCon, tr->stall?4:noErr, retBufLen);
	}
