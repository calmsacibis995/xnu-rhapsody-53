...
/*
	...
	Change History (most recent first):
	 ...
	 <USB90>	11/18/98	BT		Free up port after reset completes.
	 <USB89>	 11/4/98	BT		Fix dead device giving up on aborted error, and reading of all
									of device descriptor on second attach.
	 <USB88>	10/15/98	BT		Change to 1.1 max packet discovery
	 <USB87>	10/12/98	BT		Add power qualification to find next interface
	 <USB86>	 10/8/98	BT		Fix dead device handling
	 <USB85>	 10/3/98	BT		More reset port stuff
	 <USB84>	 9/29/98	BT		More port reset function
	 <USB83>	 9/28/98	BT		Add Cherry KB revs to errata. Add device reset framework.
									Rewrite interrupt handler.
	 <USB82>	 9/17/98	BT		Add hub errata, make work with Cherry 4 port.
	 <USB81>	 9/17/98	BT		1.1 rules for captive devices.
	 <USB80>	 9/16/98	BT		Re enable port if it strangly changes to disabled.
	 <USB79>	 9/11/98	BT		Improve are we there yet fn
	 <USB78>	 9/10/98	BT		Improved improved enumeration, check last port
	 <USB77>	 9/10/98	BT		Add are we finished query
	 <USB76>	  9/9/98	BT		Fix MCT hub strangeness
	 <USB75>	 8/25/98	BT		Isoc name changes
	 <USB74>	 8/20/98	BT		Clean up interfaces on delete
	 <USB73>	 7/23/98	BT		Check port status after 100ms wait on attach.
	 <USB72>	 7/10/98	BT		Fix fast and hot replugging issues.
	 <USB71>	 6/24/98	BT		Fix memory leak fix so it doesn't use deallocated pb
	 <USB70>	 6/24/98	BT		Fix memory leak with more than 4 ports, when deallocating
	 <USB69>	 6/24/98	DF		handle an AbortError specially.
	 <USB68>	 6/15/98	BT		Cope with 1.1 hubs speed detection
	 <USB67>	 6/13/98	TC		Pass hubDevRef rather then hubRef when insufficent power
									encountered.
	 <USB66>	 6/11/98	CJK		add reserved params to USBExpertSetDevicePowerStatus. Wanted to
									get it into the USB.i before freezing C1
	 <USB65>	 6/11/98	BT		Add power state notification message
	 <USB64>	  6/5/98	BT		Delete device may complete async now.
	 <USB63>	  6/3/98	BT		Fix device vs interface ref talking to expert
	 <USB62>	 5/28/98	CJK		change file creater to 'MPS '
	 <USB61>	 5/20/98	BT		Configure interface takes alt param
	 <USB60>	 5/20/98	BT		Fix alt usage in interface functions
	 <USB59>	 5/20/98	BT		Update to V2 param block
	 <USB58>	 5/19/98	BG		Add prototype for USBExpertRemoveDeviceDriver() and
									USBExpertInstallDeviceDriver() til they (hopefully) get
									mentioned in an appropriate header file.
	 <USB57>	 5/14/98	DF		add more status message for power changes and overcurrent
									indicators
	 <USB56>	 5/12/98	BT		New interface stuff
	 <USB55>	 4/29/98	BT		Fix reset change connection state in dead driver
	 <USB54>	 4/27/98	BT		Fix hot unplugging.
	 <USB53>	 4/27/98	DF		Use pascal strings for status & errors
	 <USB52>	 4/23/98	BT		Always remove any device on connction state change
	 <USB51>	 4/23/98	BT		Use 1 byte interrupts
	 <USB50>	 4/23/98	BT		Fix deadlock if device does not enumerate.
	 <USB49>	 4/22/98	BT		Allow not broken devices to be attached
	 <USB48>	 4/21/98	BT		Allow for other than 8 bytes max packet
	 <USB47>	 4/21/98	BT		Switch order of read device descriptor and set address
	 <USB46>	 4/20/98	BT		Use device not hub status for power supply detection.
	 <USB45>	 4/19/98	BT		Add hub status change logic.
	 <USB44>	 4/16/98	BT		Centralise error handling.
	 <USB43>	 4/15/98	BT		remove debugger
	 <USB42>	 4/15/98	BT		More error checking and robustness vs dead devices.
	 <USB41>	 4/14/98	BT		Add extra delay, some hubs seem not to appreciate things.
	 <USB40>	 4/14/98	BT		Add kill hub for when its unpo='lugged
		<39>	 4/10/98	BT		Add call to USBHubDeviceRemoved
	 <USB38>	 4/10/98	BT		Add 100ms power up delay on connect.
	 <USB37>	  4/9/98	BT		Fix reset timing and use USB.h
	 <USB36>	  4/9/98	DF		Fix to work with MasterInterfaces
		<35>	  4/8/98	BT		fix flags, add dynamic port allocation
		<34>	  4/7/98	BT		Change to new PB names
		<33>	  4/6/98	BT		Change paramblock version to 0.01
		<32>	  4/2/98	BT		Avoid C++ keywords (class)
		<31>	 3/31/98	CJK		Modify the few remaining error messages to be prefixed by "Hub
									Driver"
		<30>	 3/31/98	CJK		Change ExpertStatus strings from pstrings to cstrings.
		<29>	 3/27/98	BT		Cope with captive ports
		<28>	 3/20/98	BT		Don't use 1 byte max packet sizes.
		<27>	 3/18/98	BT		Hot unplugging
		<26>	 3/11/98	BT		Fix driver tripping over itself on too many interrupts
		<25>	  3/6/98	CJK		change hanlder to handler.  Remove DebugStr (link was
									complaining about it).
		<24>	  3/5/98	BT		Make hub more than single pluggable.
		<23>	  3/5/98	BT		Add interrupt pipe, and one hot plug
		<22>	 2/24/98	BT		Support atmel and other hubs which use different descriptor
									indexes.
		<21>	 2/23/98	BT		Make more forgiving of different hubs
		<20>	 2/10/98	BT		Fix get wIndex in get device descriptor.
		<19>	  2/8/98	BT		More Power allocation stuff
		<18>	  2/8/98	BT		Power Allocation stuff
		<17>	  2/5/98	BT		Add status notification stuff
		<16>	  2/4/98	BT		More hub driver cleanup extend
		<15>	  2/4/98	BT		Update to include new USL features and power allocation stuff.
		<14>	 1/26/98	CJK		Change to use USBDeviceDescriptor (instead of just
									devicedescriptor)
		<13>	 1/26/98	BT		Fix name changes
		<12>	 1/26/98	BT		Mangle names after design review
		<11>	 1/26/98	BT		Make expert notify public
		<10>	 1/20/98	BT		Add port indicator as debug aid, now works
		 <9>	 1/20/98	BT		Making functions static
		 <8>	 1/15/98	CJK		Change include of USL.h to USBServicesLib.h
		 <7>	 1/14/98	BT		Use new USBClassDrivers.h
		 <6>	 1/14/98	BT		Change outdated pre expert stuff to real expert
		 <5>	 1/13/98	BT		Making timers internal
		 <4>	12/18/97	BT		Add reentracy check and change notify proc notification
		 <3>	12/17/97	BT		Bring in my hub driver from the USL
...
OSStatus HubAreWeFinishedYet(void)
{
enum{
	hubTimeOut = 256
	};
...
OSStatus killHub(USBDeviceRef device)
...
void HubDriverEntry(USBDeviceRef device, USBDeviceDescriptor *desc,  UInt32 busPowerAvailable)
{
