...
/*
	...
	Change History (most recent first):
	  ...
	  <USB9>	 9/10/98	BT		Add are we finished
	  <USB8>	  9/4/98	TC		Update hubDriverNotifyProc to use version 1.1 of dispatch table.
	  <USB7>	 4/14/98	BT		Do removed devices when killed
	  <USB6>	 4/11/98	CJK		remove usbpriv.h include
	  <USB5>	  4/9/98	BT		Use USB.h
		 <4>	  3/6/98	CJK		get version info from HubClassVersion.h
		 <3>	 2/25/98	BT		Add Apple Vendor cods.
		 <2>	 2/24/98	BT		Change Apple vendor ID.
...
USBDriverDescription	TheUSBDriverDescription = 
{
	...
	// Device Info
	1452,									// vendor = Apple (1452 decimal, swapped)
	0x8001,									// product = CMD chip
	0x0001,									// version of product = 1.00 (swapped)
	...
	// Driver Info
	"\pUSBHub1Apple",						// Driver name for Name Registry
	kUSBHubClass,							// Device Class  (from USBDeviceDefines.h)
	kUSBHubSubClass,						// Device Subclass 
	kHUBHexMajorVers, 
	kHUBHexMinorVers, 
	kHUBCurrentRelease, 
