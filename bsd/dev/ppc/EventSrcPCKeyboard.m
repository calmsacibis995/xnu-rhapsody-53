...
#import <bsd/dev/ppc/event.h>
#import <bsd/dev/ppc/PPCKeymap.c>	// The default keymapping string
#import <bsd/dev/ppc/EventSrcPCKeyboard.h> 
...
unsigned int wserver_on=0;

//Make sure USB exists
id usb_UIM_object = 0;

// FIXME: Nasty hack to reinit mouse on server startup
#include "PPCMouse.h"
extern PPCMouse *_my_Mouse;
...
//		the keyboards.
{
    ...
    keyMap = [[KeyMap alloc] initFromKeyMapping:PPCDefaultKeymap
	length:sizeof PPCDefaultKeymap
	canFree:NO];
...
//		PCKeyboard device. We have to tell it to send events to us.
{
    ...
    drtn = IOGetObjectForDeviceName("PPCKeyboard0", &kbdDevice);
    if(drtn) {
	IOLog("initKeyboard: Can't find PPCKeyboard0 (%s)\n",
...
//		The event structure passed in by reference should not be freed.
{
	...
	case ADBK_SHIFT:     keyMask = NX_NEXTLSHIFTKEYMASK; break;
	case ADBK_SHIFT_R:   keyMask = NX_NEXTRSHIFTKEYMASK; break;
	case ADBK_OPTION:    keyMask = NX_NEXTLALTKEYMASK; break;
	case ADBK_OPTION_R:  keyMask = NX_NEXTRALTKEYMASK; break;
	case ADBK_CONTROL:   keyMask = NX_NEXTCTLKEYMASK; break;
	case ADBK_CONTROL_R: keyMask = NX_NEXTCTLKEYMASK; break;
    ...
    if( (event->keyCode == ADBK_CAPSLOCK)) {			// Send a capslock toggle
	if( event->goingDown != alphaLock ) {
	    [keyMap doKeyboardEvent:ADBK_CAPSLOCK
		direction:YES
		keyBits:keyState];
	    [keyMap doKeyboardEvent:ADBK_CAPSLOCK
		direction:NO
	...
	count:(unsigned int *)count
{
	...
	*returnedCount = 0;
	...
	dp->interface_addr = 0;
	dp->id = [kbdDevice handlerId];  //Right now ID is for ADB keyboard
//This line shows up very late in bootup, around time LOGIN is happening on screen
//   I think this may be the code that maps international key maps.  A.W. 12/21/98
//   Now I will call USB method to retrieve USB keyboard attached, if any.  The
//   USB method will return the same 16 + ADB ID that is in the various .keyboard
//   files in /System/Library/Keyboards/*
	
	if (usb_UIM_object != 0)
	{	
		int	adb_id;

		adb_id = [usb_UIM_object    getADBKeyboardID];
		if (adb_id != 0)
		{
			dp->id = adb_id;	//converted from USB land
			//kprintf("ADB: keyboard converted from USB is %d decimal\n", adb_id);
		}
	}
...
- (IOReturn)relinquishOwnership:(id)client
{
	...
	[self setAlphaLock:NO];
	[[self owner] updateEventFlags:0];
	deviceDependentFlags = 0;
	eventFlags = 0;
	bzero( keyState, sizeof( kbdBitVector));
