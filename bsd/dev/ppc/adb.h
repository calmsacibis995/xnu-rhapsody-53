...
struct adb_device {
    ...
    int a_flags;
    int a_type;
    void    (*a_handler)(int number, unsigned char *buffer, int bytes, void * ssp);
};
...
void        adb_unsolicited_done(adb_packet_t *response, void * ssp);
adb_request_t   *adb_next_request(void);
void        adb_poll(void);

int        adb_set_handler(struct adb_device *devp, int handler);

void	borrow_adb( void (*handler)(int number, unsigned char *packet, int count,void * ssp));
void	return_adb( void );
