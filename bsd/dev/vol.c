...
static port_t	vol_receive_port = PORT_NULL;
...
static queue_head_t vol_panel_q = {&vol_panel_q, &vol_panel_q};
...
#if 	DRIVERKIT
	    case DKIOCCHECKINSERT:
#if 0
#if	hppa
		sd_check_disks(SD_CHECK_DISKS);
#endif	hppa
#endif 0
		manual_poll = 1;
		break;
           ...
           case DKIOCCANCELAUTOMOUNT:
		mydev = *(dev_t *)data;
		vol_notify_cancel(mydev);

/* note: for hppa, if we go with the different vol driver behavior,
 * this function needs to be taken from the 3.2 HP kernel
 */
#if 0
#ifdef hppa
		sd_ignore_vol(mydev);
	...
	int flags)
{
	...
	if(vol_check_port == (kern_port_t)NULL) {
		 ...
		 */
		vnep = (vne_t)kalloc(sizeof(struct vol_notify_entry));
...
#endif	SUPPORT_PORT_DEVICE
	    default:
	    	break;			// can't happen, avoid compiler
					//    warning
...
kern_return_t vol_panel_request(vpt_func fnc,
	...
	*tag = panel_msgp->pr_tag;
	...
	if(response_type == PR_RT_NONE)
		return(KERN_SUCCESS);
	vpe = (vpe_t)kalloc(sizeof(struct vol_panel_entry));
	if(vpe == NULL)
...
kern_return_t vol_panel_remove(int tag) {
	...
	kern_return_t krtn = KERN_SUCCESS;
	vpe_t vpe;
...
void vol_thread() {
	...
	while(1) {
		msgp->ps_header.msg_local_port = vol_receive_port;
...
void vol_start_thread()
{
	...
	krtn = ipc_port_alloc(kernel_task->itk_space, &name, &vol_local_port);
	if(krtn) {
		printf("vol_start_thread: port_alloc returned %d\n", 
			krtn);
		return;
	}
	vol_receive_port = vol_local_port->ip_receiver_name;
	ip_unlock(vol_local_port);
	kernel_thread(kernel_task, vol_thread, (void *)0);
